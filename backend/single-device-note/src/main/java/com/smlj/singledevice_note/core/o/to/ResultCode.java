package com.smlj.singledevice_note.core.o.to;

import lombok.AllArgsConstructor;
import lombok.Getter;

// https://github.com/kaclok/springboot-vue/blob/master/yudao-framework/yudao-common/src/main/java/cn/iocoder/yudao/framework/common/exception/enums/GlobalErrorCodeConstants.java#L17
@Getter
@AllArgsConstructor
public enum ResultCode {
    RC199(199, "失败"),
    RC200(200, "成功"),

    // 协议参数传输
    RC400(400, "请求参数不正确"),
    RC402(401, "请求参数缺失"),
    RC403(403, "无访问权限"),
    RC404(404, "请求未找到"),

    // ========== 服务端错误段 ==========
    RC500(500, "系统异常"),
    RC501(501, "功能未实现/未开启"),
    RC502(502, "错误的配置项"),

    RC10000(10000, "access token过期"),
    RC10001(10001, "refresh token过期"),
    RC10002(10002, "access token不存在(未登录)"),
    RC10005(10005, "token校验不正确"),

    // 公共业务（参数校验/重复/为空）
    RC10101(10101, "请求参数为空或不合法"),
    RC10102(10102, "数据已存在，禁止重复录入"),
    RC10103(10103, "目标数据不存在"),

    // 文件
    RC10104(10104, "文件超过限制大小(单个300MB,总共1000MB)"),

    // 文件服务（MinIO）
    RC10501(10501, "文件记录不存在"),
    RC10502(10502, "文件未上传完成，无法下载"),
    RC10503(10503, "MinIO中文件对象不存在"),
    RC10504(10504, "文件已删除"),
    RC10505(10505, "生成预签名URL失败"),
    RC10506(10506, "文件上传失败"),
    RC10507(10507, "文件下载失败"),
    RC10508(10508, "文件删除失败"),

    // 验证码
    RC10201(10201, "验证码不正确"),

    // 用户
    RC10301(10301, "用户不存在"),
    RC10302(10302, "用户所在的部门不存在"),
    RC10303(10303, "密码错误"),
    RC10304(10304, "账号已存在，禁止重复创建"),
    RC10305(10305, "分配的角色不存在或不合法"),
    RC10306(10306, "不能停用当前登录账号自身"),
    RC10307(10307, "无操作权限"),

    // 请求签名校验
    RC10401(10401, "请求签名参数缺失"),
    RC10402(10402, "请求签名校验失败"),
    RC10403(10403, "请求已过期"),
    RC10404(10404, "重复请求(nonce已存在)"),

    // 保持在最后
    RC_1(-1, "未知错误");

    // 自定义状态码
    private final int code;
    // 自定义描述
    private final String message;
}
